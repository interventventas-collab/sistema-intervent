...
public class CafeVentasController : ControllerBase
{
    ...
    public async Task<IActionResult> SumarAlMapa(int id, [FromBody] SumarAlMapaRequest? req)
    {
        ...
        var r = await _ventaMapeo.SumarVentaAsync(v, req?.Direccion, req?.Link);
        return Ok(new
        {
            ok = r.Ok,
            yaEstaba = r.YaEstaba,
            ...
            direccionSugerida = r.DireccionSugerida,
            nombre = r.Nombre,
    ...
    // 2026-09-21: el escaneo de etiqueta MeLi se mudó a MeliDepositoController
    // (página aparte "Órdenes MeLi · Depósito"), separado del armado de pedidos.
